Function LIB-Send-Confirmation{
  ...
  do {
    ...
    try {
      ...
      if ($mode -eq "Start"){
        $Mail.Subject = "Foundation Addon Provisioning Started for $($datavar.pocname)";
        $body += "<h2>Foundation Addon Provisioning Started for $($datavar.pocname)</h2>";
      } elseif($mode -eq "FailedVPN") {
        $Mail.Subject = "Foundation Addon Provisioning Failed for $($datavar.pocname): VPN Issue";
        ...
        $Mail.Subject = "Foundation Addon Provisioning Failed for $($datavar.pocname): Images not present.";
        $body += "<h2>Foundation Addon Provisioning Failed for $($datavar.pocname)</h2>";
        $body += "<b>Image not present</b>"
      } elseif($mode -eq "Locked"){
        $Mail.Subject = "Foundation Addon Provisioning is locked for $($datavar.pocname)";
        ...
        $Mail.Subject = "Foundation Addon Provisioning is queued for $($datavar.pocname)";
        ...
        $Mail.Subject = "Foundation Addon Provisioning failed validation for $($datavar.pocname)";
        ...
        $Mail.Subject = "Foundation Addon Provisioning Single Threaded mode active, request $($datavar.pocname) queued";
        $body += "<h2>Foundation Addon Provisioning is queued for $($datavar.pocname)</h2>";
        $body += "<b>The item will be submitted for auto queue once validated.</b>"
        ...
        $Mail.Subject = "Foundation Addon Provisioning Finished for $($datavar.pocname)";
